...
import { getTemplateWebTemplate } from "@/services/templates"
...
export function EditPatient() {
  ...
  const [templateData, setTemplateData] = useState<Record<string, unknown>>(example)
  ...
  const loadVitals = useCallback(async (ehrId: string) => {
    try {
      ...
      const compositions = await getVitalsCompositions(ehrId, selectedTemplate)
  ...
  // Reload vitals when template changes
  useEffect(() => {
    if (formData.ehrId) {
  ...
  useEffect(() => {
    const loadTemplateData = async () => {
      try {
        // If we're using the default template, use the example data
        if (selectedTemplate === "jitendra.choudhary.vitals.v1") {
          setTemplateData(example);
          return;
        }

        // Otherwise, fetch the template data from the API
        const webTemplate = await getTemplateWebTemplate(selectedTemplate);
        setTemplateData(webTemplate);
      } catch (error) {
        console.error("Error loading template data:", error);
        toast({
          title: "Error",
          description: "Failed to load template data. Using default template.",
          variant: "destructive",
        });
        // Fall back to the example template
        setTemplateData(example);
      }
    };
  ...
  const handleSaveVitals = async (composition?: VitalsComposition) => {
    ...
    try {
      if (selectedVitals?.uid) {
        ...
        await updateVitalsComposition(formData.ehrId, selectedVitals.uid, composition, selectedTemplate)
      } else {
        // Create new vitals
        await saveVitalsComposition(formData.ehrId, composition, selectedTemplate)
      }
